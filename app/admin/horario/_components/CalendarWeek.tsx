...
export default function CalendarWeek({
  ...
  return (
    <div className="bg-surface-container-lowest rounded-sm ambient-shadow overflow-hidden">
      {/* Cabecera con días */}
      <div className="grid grid-cols-[60px_repeat(7,1fr)] bg-surface-container-low border-b border-outline-variant/30">
        <div />
        {days.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className="px-3 py-3 text-center border-l border-outline-variant/20"
            >
              <div className="font-sans text-[10px] tracking-[0.18em] uppercase text-on-surface/55 font-semibold mb-1">
                {WEEKDAY_LABELS[i]}
              </div>
              <div
      ...
      {weekSesiones.some((s) => !s.hora_inicio) && (
        <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-outline-variant/30 bg-surface-container-low/40">
          <div className="px-2 py-2 font-sans text-[10px] uppercase tracking-[0.15em] text-on-surface/45 text-right pr-3 flex items-center justify-end">
            Todo el día
          </div>
          {days.map((d, i) => {
            ...
            return (
              <div
                ...
                className="border-l border-outline-variant/20 p-1 min-h-[34px] flex flex-col gap-1"
              >
                {allDay.map((s) => {
        ...
        <div className="relative">
          {hours.map((h) => (
            <div
              ...
              className="font-sans text-[10px] text-on-surface/45 tabular-nums text-right pr-2 pt-0.5 border-b border-outline-variant/15"
            >
