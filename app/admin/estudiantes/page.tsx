...
const estadoStyle: Record<string, string> = {
  ...
  graduado: "bg-primary-container text-on-primary-container border border-primary-container",
};

const estadoLabel: Record<string, string> = {
  activo: "Activo", inactivo: "Inactivo", graduado: "Graduado",
};
...
const editEstudianteSchema = z.object({
  ...
  fecha_inscripcion: z.string().min(1, "La fecha de inscripción es obligatoria"),
...
export default function EstudiantesPage() {
  ...
  const form = useForm<EstudianteForm>({
    ...
    defaultValues: {
      name: "", email: "", password: "", cedula: "", telefono: "",
      fecha_nacimiento: "", genero: undefined, curso_id: undefined,
  ...
  const fetchEstudiantes = async () => {
    try {
      const res = await fetch(`${process.env.API_URL}api/admin/estudiantes`, { headers: getAuthHeaders() });
      if (!res.ok) { setError("No se pudo cargar la lista de estudiantes."); return; }
  ...
  useEffect(() => { fetchEstudiantes(); fetchCursos(); }, []);
  ...
  const onEdit = async (data: EditEstudianteForm) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.API_URL}api/admin/estudiantes/${editTarget.id}`, {
        method: "PUT",
        headers: getAuthHeaders(),
        body: JSON.stringify(body),
      });
  ...
  const filtered = useMemo(() => {
    return estudiantes.filter((e) => {
      ...
      const matchCurso =
        filterCurso === "todos" ||
        (filterCurso === "sin_curso" ? !e.curso : String(e.curso?.id) === filterCurso);
  ...
  const hasFilters = filterEstado !== "todos" || filterCurso !== "todos" || search !== "";
    ...
    <div className="relative min-h-full bg-surface">
      ...
      <div className="relative z-10 px-10 py-10 max-w-8xl">
        ...
        <div className="mb-10 flex items-end justify-between">
          ...
          <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { form.reset(); setSubmitError(""); } }}>
            <DialogTrigger asChild>
              <Button className="gap-2"><Plus className="w-4 h-4" />Nuevo estudiante</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="font-serif font-semibold text-2xl text-on-surface">Registrar estudiante</DialogTitle>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4">
                <div className="grid gap-2">
                  ...
                  <Input id="name" placeholder="Ej: Juan Pérez" {...form.register("name")} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input id="cedula" placeholder="V-12345678" {...form.register("cedula")} />
                    {form.formState.errors.cedula && <p className="text-sm text-destructive">{form.formState.errors.cedula.message}</p>}
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="telefono">Teléfono</Label>
                    <Input id="telefono" placeholder="0412-1234567" {...form.register("telefono")} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label>Género</Label>
                    <Select value={form.watch("genero") ?? ""} onValueChange={(v) => form.setValue("genero", v as EstudianteForm["genero"])}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label>Curso</Label>
                    <Select value={form.watch("curso_id") ?? ""} onValueChange={(v) => form.setValue("curso_id", v)}>
                      <SelectTrigger className="w-full"><SelectValue placeholder="Sin curso" /></SelectTrigger>
                      <SelectContent>
                        {cursos.map((c) => (
                  ...
                  <div className="grid gap-2">
                    <Label>Estado *</Label>
                    <Select value={form.watch("estado")} onValueChange={(v) => form.setValue("estado", v as EstudianteForm["estado"])}>
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="fecha_inscripcion" type="date" {...form.register("fecha_inscripcion")} />
                  {form.formState.errors.fecha_inscripcion && <p className="text-sm text-destructive">{form.formState.errors.fecha_inscripcion.message}</p>}
                </div>
        ...
        <div className="grid grid-cols-3 gap-5 mb-10 max-w-3xl">
          {[
            { label: "Total",     value: counts.total,     icon: Users,         color: "text-on-primary-container",   glow: "bg-primary-container" },
            { label: "Activos",   value: counts.activos,   icon: BookOpen,      color: "text-on-primary-container",   glow: "bg-primary-container/70" },
            { label: "Graduados", value: counts.graduados, icon: GraduationCap, color: "text-on-secondary-container", glow: "bg-secondary-container" },
          ].map((s) => (
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasFilters && (
              <button
                onClick={() => { setSearch(""); setFilterEstado("todos"); setFilterCurso("todos"); }}
          ...
          <div className="bg-surface-container-low rounded-sm overflow-hidden ambient-shadow">
            ...
            {filtered.length === 0 ? (
              <div className="flex items-center justify-center py-16 font-sans text-sm text-muted-foreground">
                {hasFilters ? "No hay estudiantes con esos filtros." : "No hay estudiantes registrados."}
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-outline-variant/40">
                      {["Estudiante", "Cédula", "Curso", "Inscripción", "Estado", ""].map((h) => (
                        <th key={h} className="text-left px-6 py-3.5 font-sans text-xs tracking-[0.15em] uppercase text-on-surface/55 font-semibold">
                          {h}
                        </th>
                      ))}
                    </tr>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-primary-container flex items-center justify-center shrink-0">
                              <span className="font-sans text-sm font-bold text-on-primary-container">{getInitials(e.nombre)}</span>
                            </div>
                            <div>
                              <p className="font-sans font-semibold text-on-surface text-sm">{e.nombre}</p>
                              <p className="font-sans text-xs text-muted-foreground">{e.user.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 font-mono text-sm text-muted-foreground tracking-wide">{e.cedula}</td>
                        <td className="px-6 py-4 font-sans text-sm">
                          {e.curso
                            ? <span className="text-on-surface">{e.curso.nombre}</span>
                            : <span className="text-on-surface/40 italic text-xs">Sin curso</span>
                          }
                        </td>
                        <td className="px-6 py-4 font-sans text-sm text-muted-foreground whitespace-nowrap">
                          {new Date(e.fecha_inscripcion).toLocaleDateString("es-VE", { day: "2-digit", month: "short", year: "numeric" })}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full font-sans text-xs font-semibold ${estadoStyle[e.estado]}`}>
      ...
      <Dialog open={!!editTarget} onOpenChange={(v) => { if (!v) setEditTarget(null); }}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="font-sans text-sm text-muted-foreground">
              Modifica los datos de <strong>{editTarget?.nombre}</strong>.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={editForm.handleSubmit(onEdit)} className="grid gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                {editForm.formState.errors.cedula && <p className="text-sm text-destructive">{editForm.formState.errors.cedula.message}</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="edit-telefono" placeholder="0412-1234567" {...editForm.register("telefono")} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-fecha_nacimiento">Fecha de nacimiento</Label>
                <Input id="edit-fecha_nacimiento" type="date" {...editForm.register("fecha_nacimiento")} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => editForm.setValue("genero", v as EditEstudianteForm["genero"])}
                >
                  <SelectTrigger className="w-full"><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="masculino">Masculino</SelectItem>
              ...
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => editForm.setValue("estado", v as EditEstudianteForm["estado"])}
                >
                  <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="activo">Activo</SelectItem>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label>Curso</Label>
                <Select
                  value={editForm.watch("curso_id") ?? ""}
                  onValueChange={(v) => editForm.setValue("curso_id", v)}
                >
                  <SelectTrigger className="w-full"><SelectValue placeholder="Sin curso" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid gap-2">
                <Label htmlFor="edit-fecha_inscripcion">Fecha de inscripción *</Label>
                <Input id="edit-fecha_inscripcion" type="date" {...editForm.register("fecha_inscripcion")} />
                {editForm.formState.errors.fecha_inscripcion && <p className="text-sm text-destructive">{editForm.formState.errors.fecha_inscripcion.message}</p>}
              </div>
            </div>
