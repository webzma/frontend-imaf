...
export default function ProfesorPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.API_URL}api/me`, { headers })
      ...
      .then((me) => {
        setUserId(me.id);
        return Promise.all([
          fetch(`${process.env.API_URL}api/admin/profesores/${me.id}`, { headers })
      ...
      .then(([perfilData, cursosData]) => {
        ...
        const misCursos = Array.isArray(cursosData)
          ? cursosData.filter((c: Curso & { profesor?: { id: number } }) =>
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${process.env.API_URL}api/profesor/perfil/${userId}`, {
        method: "PUT",
  ...
  const totalEstudiantes = cursos.reduce((s, c) => s + (c.estudiantes?.length ?? 0), 0);
    ...
    <div className="relative min-h-screen bg-surface">
      ...
      <header className="relative z-10 px-8 py-5 flex items-center justify-between border-b border-outline-variant/30">
        <div className="flex items-center gap-3">
          ...
          <span className="font-sans font-semibold text-on-surface tracking-tight">IMAF</span>
          <span className="font-sans text-[10px] text-muted-foreground/60 tracking-[0.2em] uppercase">Profesor</span>
        </div>
        <button
      ...
      <div className="relative z-10 px-8 py-10 max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-sans text-[10px] tracking-[0.22em] uppercase text-primary/70 font-medium">Mi perfil</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-surface-container-lowest rounded-sm ambient-shadow p-6">
              {loading ? (
                ...
                <div className="flex flex-col items-center text-center gap-3">
                  ...
                  <div>
                    <h2 className="font-serif font-light text-2xl text-on-surface">{perfil.user.name}</h2>
                    <p className="font-sans text-sm text-muted-foreground">{perfil.user.email}</p>
                  </div>
                  {perfil.titulo && (
                    <span className={`inline-flex items-center px-3 py-1 rounded-full font-sans text-xs font-semibold ${tituloStyle[perfil.titulo] ?? "bg-muted text-muted-foreground"}`}>
                  ...
                  <div className="w-full pt-4 border-t border-outline-variant/30 space-y-2.5 text-left">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-mono text-muted-foreground text-xs">{perfil.cedula}</span>
                    </div>
                    {perfil.telefono && (
                      <div className="flex items-center gap-2 text-sm">
                        <Phone className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
                        <span className="font-sans text-muted-foreground text-sm">{perfil.telefono}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      <Mail className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
                      <span className="font-sans text-muted-foreground text-xs break-all">{perfil.user.email}</span>
                    </div>
                    {perfil.especialidad && (
                      <div className="flex items-center gap-2">
                        <BadgeCheck className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
                        <span className="font-sans text-muted-foreground text-sm">{perfil.especialidad}</span>
                      </div>
                    )}
                    {perfil.departamento && (
                      <div className="flex items-center gap-2">
                        <Building2 className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
                        <span className="font-sans text-muted-foreground text-sm">{perfil.departamento}</span>
                      </div>
                    )}
                  </div>

                  <Button variant="outline" size="sm" className="w-full gap-2 mt-2" onClick={() => setEditing(!editing)}>
                    <Pencil className="w-3.5 h-3.5" />Editar perfil
                  </Button>
                </div>
              ) : (
                <p className="font-sans text-sm text-muted-foreground text-center">No se pudo cargar el perfil.</p>
              )}
            </div>
            ...
            {!loading && (
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-surface-container-lowest rounded-sm ambient-shadow p-4 text-center">
                  <p className="font-sans text-2xl font-semibold text-on-surface">{cursos.length}</p>
                  <p className="font-sans text-[10px] tracking-[0.15em] uppercase text-muted-foreground mt-0.5">Cursos</p>
                </div>
                <div className="bg-surface-container-lowest rounded-sm ambient-shadow p-4 text-center">
                  <p className="font-sans text-2xl font-semibold text-on-surface">{totalEstudiantes}</p>
                  <p className="font-sans text-[10px] tracking-[0.15em] uppercase text-muted-foreground mt-0.5">Alumnos</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {editing && (
              <div className="bg-surface-container-lowest rounded-sm ambient-shadow p-6">
                <h3 className="font-serif font-light text-xl text-on-surface mb-5">Editar información</h3>
                <div className="grid gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="telefono">Teléfono</Label>
                      <Input id="telefono" placeholder="0412-1234567" value={form.telefono} onChange={(e) => setForm((f) => ({ ...f, telefono: e.target.value }))} />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="especialidad">Especialidad</Label>
                      <Input id="especialidad" placeholder="Ej: Matemáticas" value={form.especialidad} onChange={(e) => setForm((f) => ({ ...f, especialidad: e.target.value }))} />
                    </div>
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="departamento">Departamento</Label>
                    <Input id="departamento" placeholder="Ej: Ciencias" value={form.departamento} onChange={(e) => setForm((f) => ({ ...f, departamento: e.target.value }))} />
                  </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-sans text-xs tracking-[0.18em] uppercase text-on-surface/55 font-semibold">Mis cursos</h3>
              </div>

              {loading ? (
                <div className="space-y-3">
                  {[1, 2].map((i) => (
                    <div key={i} className="bg-surface-container-lowest rounded-sm ambient-shadow p-5">
                ...
                <div className="bg-surface-container-lowest rounded-sm ambient-shadow flex flex-col items-center justify-center py-14 gap-3">
                  ...
                  <p className="font-sans text-sm text-muted-foreground">No tienes cursos asignados aún.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {cursos.map((curso) => {
                    const activos = curso.estudiantes?.filter((e) => e.estado === "activo").length ?? 0;
                    const total = curso.estudiantes?.length ?? 0;
                    return (
                      <div key={curso.id} className="bg-surface-container-lowest rounded-sm ambient-shadow overflow-hidden">
                        <div className="h-1 gradient-primary" />
                        <div className="p-5">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h4 className="font-serif font-light text-xl text-on-surface">{curso.nombre}</h4>
                              <span className="inline-flex items-center gap-1 font-mono text-xs font-bold text-on-primary-container bg-primary-container px-2 py-0.5 rounded-sm mt-1">
                                <Hash className="w-2.5 h-2.5" />{curso.codigo}
                              </span>
                            </div>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full font-sans text-xs font-semibold ${
                              curso.estado === "activo" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                            }`}>
                          ...
                          {curso.descripcion && (
                            <p className="font-sans text-sm text-muted-foreground line-clamp-2 mb-3">{curso.descripcion}</p>
                          )}

                          <div className="flex items-center gap-5 pt-3 border-t border-outline-variant/30">
                            <div className="flex items-center gap-1.5 text-muted-foreground text-sm">
                              <Star className="w-3.5 h-3.5" />
                              <span className="font-sans">{curso.creditos} créditos</span>
                            </div>
                            <div className="flex items-center gap-1.5 text-muted-foreground text-sm">
                              <Users className="w-3.5 h-3.5" />
                              <span className="font-sans">
                                <span className="font-semibold text-on-surface">{total}</span> estudiantes
                                {total > 0 && <span className="text-emerald-600 ml-1">({activos} activos)</span>}
                              </span>
                            </div>
                          </div>
                        </div>
