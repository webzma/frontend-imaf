...
interface Estudiante {
  ...
  /** Estudiante archivado: sigue en el historial del curso. */
  deleted_at: string | null;
  user: { email: string };
}

const PAGO_LABEL: Record<Estudiante["estado_pago"], string> = {
  aprobado: "Pago al día",
  pendiente: "Pago pendiente",
  reprobado: "Pago rechazado",
};
...
export default function CursoDetailPage({
    ...
    try {
      const res = await fetch(
        `${process.env.API_URL}api/profesor/estudiantes/${estudianteId}/aprobacion-curso`,
        {
          method: "PATCH",
          headers: getAuthHeaders(),
          // El estudiante puede estar en otro curso ahora; la aprobación es
          // la de este curso.
          body: JSON.stringify({
            estado_aprobacion_curso: estado,
            curso_id: Number(id),
          }),
        },
      );
  ...
  // Solo se aprueba el curso a quien pagó y no está archivado; los demás
  // figuran en la lista pero no cuentan como pendientes de aprobación.
  const pendientesAprobacion = curso.estudiantes.filter(
    (e) =>
      e.estado_aprobacion_curso === "pendiente" &&
      e.estado_pago === "aprobado" &&
      !e.deleted_at,
  ).length;
        ...
        stats={[
          {
            label: "Inscritos",
            value: inscritos,
            sub: `de ${curso.limite_cupo} cupos`,
            icon: Users,
            tone: "primary",
          },
          {
          ...
          {
            label: "Aprobados",
            value: curso.estudiantes.filter(
              (e) => e.estado_aprobacion_curso === "aprobado",
            ).length,
            sub: "curso completado",
            icon: Users,
            tone: "success",
          },
        ]}
